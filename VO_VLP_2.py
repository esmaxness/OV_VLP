...
class VisualOdometry:
    def __init__(self, camera_matrix, dist_coeffs, beacon_world_positions):
        ...
        self.current_position = (-23.9,29.85) #-53.80,29.95(30.20, 35.7)  # [x, y, z]
        ...
        self.position_history = [(-23.9,29.85)]  # Inicializar con la posición inicial
    ...
    def estimacion_real_position(self, deltas):
        ...
        if abs(self.current_yaw) > np.radians(180):
    ...
    def process_beacon_detection(self, beacon_id, pixel_position):
        ...
        Returns:
            delta: Devuelve el delta que se desplazó en cm
        """
        self.frame_count += 1
        ...
        self.beacon_pixel_history[beacon_id].append({
            ...
            'pixel_pos': pixel_position,  # Usar el punto corregido
    ...
    def convert_imageCoordinate(self, beacon_detections):
        
        beacon_ids = list(beacon_detections.keys())

        for id in beacon_ids:
            x =  beacon_detections[id][0]
            y = beacon_detections[id][1]
            beacon_detections[id][0] = y # x = y
            beacon_detections[id][1] = -x  #y = -x
        
    def apply_undistor(self,beacon_detections):

        beacon_ids = list(beacon_detections.keys())

        for id, pos in beacon_detections.items():
            
            # Aplicar corrección de distorsión al punto detectado
            undistorted_position = self.undistort_point(pos)
            print(undistorted_position)
            ...
            beacon_detections[id] = undistorted_pixel
    ...
    def process_multiple_beacons(self, beacon_detections):
        ...
        # Convertir en coordenadas Imagen
        self.convert_imageCoordinate(beacon_detections)

        # Corregir los pixeles usando los coeficientes de distorción
        self.apply_undistor(beacon_detections)
        print(f"beacon_detections after undistor:{beacon_detections}")
        ...
        for beacon_id, pixel_pos in beacon_detections.items():
            ...
            if deltas is not None:
                ...
                deltas_rotados = np.linalg.inv(self.current_rotation) @ deltas
    ...
    def update_position(self, beacon_detections):
        ...
        if beacon_detections:
            print(f"beacon_detections:{beacon_detections}")
